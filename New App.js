...
const myAPIClient = generateClient('https://cmao5s9ach.execute-api.eu-west-3.amazonaws.com/dev');
...
const AudioControls = ({ isAudioResponse, filterMessageObjects, messages, setMessages, handleBackendResponse }) => {
  ...
  Amplify.configure({
    ...
    API: {
      endpoints: [
        {
          name: "api", // Use the actual API name here
  ...
  const uploadAudio = async () => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const response = await myAPIClient.post("api", "/get-answer", {
          headers: {
